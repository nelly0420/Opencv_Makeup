...
def apply_blush(image, prdCode):
    ...
    for k, d in enumerate(faces):
        ...
        for cheek_center in [left_cheek_center, right_cheek_center]:
            ...
            alpha_channel = cv2.GaussianBlur(alpha_channel, (45, 45), 0)  # 블러 커널 크기 조정
            alpha_channel = cv2.medianBlur(alpha_channel, 21)  # 미디언 블러 커널 크기 조정

            # 블러셔 색상 이미지 생성
            blush = np.zeros_like(image_bgra, dtype=np.uint8)
            blush[:, :, :3] = blush_color
            blush[:, :, 3] = alpha_channel
