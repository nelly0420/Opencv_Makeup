...
# dlib 초기화
detector = dlib.get_frontal_face_detector() #-> detector는 정면을 활용, 네모난 크기 잡아주면
predictor = dlib.shape_predictor("shape_predictor_68_face_landmarks.dat") #-> 네모난 크기 안에서 랜드마크 잡아줌
blush_alpha = 0.3  # 투명도
...
def apply_blush(image, prdCode):
    ...
    for k, d in enumerate(faces):
        ...
        for cheek, name in zip([left_cheek, right_cheek], ["left_cheek", "right_cheek"]):
            ...
            mask = np.zeros(image.shape[:2], dtype=np.uint8)#-> 볼 영역만 채우게 하려고 볼터치영역 이외는 다 0으로 설정. zeros->내가 직접 영역을 설정해서 0으로 만들어줘야함.
            pts = cheek.reshape((-1, 1, 2))
            cv2.fillPoly(mask, [pts], 255) #-> 다각형을 채우는 함수
            # 블러쉬 컬러 채널 생성 및 적용-> 블러쉬 색 적용
            blush = np.zeros_like(image_bgra) #모든 값이 0으로 초기화, 아무런 색상이 적용되지 않음. zeres_loke->다음에 나오는 배열만큼0으로 만들기
    ...
    return image
## 블러셔는 각 끝점의 중심 값을 찾아서 그 부위로부터 점점 옅어지게 볼 영역 적용해서 블러셔 적용해보기